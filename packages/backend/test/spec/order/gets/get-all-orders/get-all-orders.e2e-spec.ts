...
describe('Get all orders', () => {
    ...
    it('should return [] if the no Order in the db',async () => {
        const orderResponse = await request(app.getHttpServer()).get('/api/orders');
        expect(orderResponse.status).toBe(200);
        expect(orderResponse.body).toEqual([]);
    });

    it('should return all orders in the db',async () => {
