...
describe('Cancel order', () => {
    ...
    it('should return Error(404) if the order is not found',async () => {
        const orderResponse = await request(app.getHttpServer()).patch(`/api/orders/${orderId}/cancel`);
        expect(orderResponse.status).toBe(404);
    });

    it('should change the order status to canceled', async () => {
        const { updateOrderResponse } = await whenUpdatingAOrderStatus(app, order, 'cancel');
